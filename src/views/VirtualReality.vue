...
onBeforeMount(() => {
  ...
  store.state.app.isTransparent = 'bg-white';
});
onBeforeUnmount(() => {
  store.state.app.layout = 'default';
  ...
  if (store.state.app.isPinned === false) {
    const sidenav_show = document.querySelector('.g-sidenav-show');
    sidenav_show.classList.remove('g-sidenav-hidden');
    sidenav_show.classList.add('g-sidenav-pinned');
    store.state.app.isPinned = true;
  }
  store.state.app.isTransparent = 'bg-transparent';
});
</script>
<template>
  <div class="mt-3">
    <navbar
      ...
      :class="`${store.state.app.isNavFixed ? store.state.app.navbarFixed_class : ''} ${
        store.state.app.isNavFixed ? 'bg-white' : 'bg-success'
      }`"
    />
  </div>
  <div
