...
const darkMode = () => {
  if (store.state.app.darkMode) {
    store.state.app.darkMode = false;
    setSidebarType('bg-white');
    deactivateDarkMode();
    return;
  } else {
    store.state.app.darkMode = true;
        ...
        <div class="mt-2 mb-5 d-flex">
          ...
          <div class="form-check form-switch ps-0 ms-auto my-auto">
            <input
              ...
              type="checkbox"
              :checked="store.state.app.darkMode"
              @click="darkMode"
            />
          </div>
        </div>
        <a class="btn bg-gradient-dark w-100" href="https://www.creative-tim.com/product/vue-argon-dashboard"
          >Free Download</a
        >
        <a
